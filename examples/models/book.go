...
func (b *BookBrick) InsertOne(args *Book) (sql.Result, error) {
	return b.db.NamedExec(`INSERT INTO book (name, content, price) VALUES (:name, :content, :price)`, args)
}

// UpdatePrice generated by sqlbrick, update data in database.
func (b *BookBrick) UpdatePrice(args *Book) (sql.Result, error) {
	return b.db.NamedExec(`UPDATE book SET price = :price, content = :content WHERE id = :id`, args)
}

// ComplexUpdate generated by sqlbrick, update data in database.
func (b *BookBrick) ComplexUpdate(args *Book) (sql.Result, error) {
	return b.db.NamedExec(`UPDATE book SET price=(SELECT price FROM book, user WHERE book.uid=user.id)
WHERE book.price <= :price AND name = :name`, args)
}
...
func (b *BookBrick) SelectById(dest interface{}, id interface{}) error {
	rows, err := b.db.NamedQuery(`SELECT * FROM book WHERE id = :id ORDER BY name ASC`, id)
	if err != nil {
		return err
	}
...
func (b *BookBrick) DeleteById(id interface{}) (sql.Result, error) {
	return b.db.NamedExec(`DELETE FROM book WHERE id = :id`, id)
}
